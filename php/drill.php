    ...
    $email = $_POST['email'];
        ...
        if(mysqli_num_rows($queryResSel)>0){
            while($row  = mysqli_fetch_assoc($queryResSel)){
                ...
                if($q >= $quantity){
                    $newQuant = $q - $quantity;
                    $book = "INSERT INTO orders VALUE('$quantity','BOSCH GBH 2-26 RE ROTARY HAMMER','$name','$address','$pin','$mobile','$email')";
                    ...
                    echo "Product : BOSCH GBH 2-26 RE ROTARY HAMMER Drill<br>";
                    echo "Name : ".$name."<br>";
